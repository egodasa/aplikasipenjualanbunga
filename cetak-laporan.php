...
if(isset($_GET['laporan'])){
    switch($_GET['laporan']){
        ...
        case "bulanan" : 
            ...
            $dataTable = $db->sql('select a.id_pesan, month(a.tgl_pesan) as bulan, a.jumlah, c.harga, c.nm_produk, (a.jumlah*c.harga) as `total_harga` from pesanan a join user b on a.id_user = b.id_user join produk c on a.id_produk = c.id_produk where month(a.tgl_pesan) = month(now()) and year(a.tgl_pesan) = year(now())')->many();
        break;
        case "tahunan" : 
            $tableConf = array(
                array(
